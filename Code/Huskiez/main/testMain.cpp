...
{
    if(fabs(targetValue - getAngle()) < 3) // in the case that the robot did not center and gyroDrift would be too great
...
void turnTo(double degrees, double speed = 40)
{
    ...
    while(true)
    {
        ...
        if (fabs(error) > 180)
            error =  sgn(error) * (360 - fabs(error));
        ...
        if(fabs(error) <= 0.3 && fabs(lastError) <= .3) break; // Break statement

        if(fabs(motorPower) > fabs(speed))
...
void drive(double inches, double speed, int cycles = 15, int timeSlice = 50, double heading = -121, bool turn = true)
{
    ...
    Brain.resetTimer();
    int theoreticalTime = fabs(rots / (speed * 2) / 60);

    // PID Loop
    while (fabs(FrontRight.rotation(rotationUnits::rev)) < fabs(rots)/* && Brain.timer(timeUnits::msec) < theoreticalTime + 1000*/)
    {
        ...
        motorPower = fabs(P) + fabs(I) - fabs(D); // Manually setting motor power

        if (fabs(motorPower) > fabs(speed)) // Limiting motor power to top speed
            motorPower = fabs(speed);
