...
void Drivetrain::driveTask()
{
  ...
  while(true)
  {
    ...
    if (std::abs(cont.get_analog(E_CONTROLLER_ANALOG_LEFT_Y) * cont.get_analog(E_CONTROLLER_ANALOG_LEFT_Y)) > 100)
      leftSide = cont.get_analog(E_CONTROLLER_ANALOG_LEFT_Y);
    else
      leftSide = 0;
    if (std::abs(cont.get_analog(E_CONTROLLER_ANALOG_RIGHT_Y)*cont.get_analog(E_CONTROLLER_ANALOG_LEFT_Y)) > 100)
